...
export const filterByMemory = (selectedMemory, data) => {
    return selectedMemory.length
        ? data.filter(({ gender }) => selectedMemory.includes(gender))
        : data;
};

export const filterByStorage = (selectedStorage, data) => {
    return selectedStorage.length
        ? data.filter(({ weight }) => selectedStorage.includes(weight))
        : data;
};
